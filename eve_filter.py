...
def gridify(gridname, xs, ys, ts, ps, timebin=10, colorise=False, threshold=250, xmax=-1, ymax=-1, tmax=-1):
    total_time_diffs = []
    if xmax==-1:
        xmax = np.max(xs)
    if ymax==-1:
...
time_diffs = gridify(f"{path}/filtered_events_image_{timebin}ms_color.npz", filtered_xs, filtered_ys, filtered_ts, filtered_ps, timebin=timebin, colorise=True, threshold=150, xmax=xmax, ymax=ymax, tmax=tmax)
...
plt.figure()
plt.hist(list(time_diffs), bins=np.arange(0, 10000, 10))
